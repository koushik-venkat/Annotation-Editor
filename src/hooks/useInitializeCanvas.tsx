...
const useInitializeCanvas = () => {
  ...
  const initializeCanvas = (
    ...
    fabric.Image.fromURL(urlImage, (img) => {
      const scaleHeight = heightCanvas / img.height!;
      const scaleWidth = widthCanvas / img.width!;
      const scale = Math.min(scaleHeight, scaleWidth);

      console.log('scale dimensions', scaleHeight, scaleWidth);

      const finalHeight = img.height! * scale;
      const finalWidth = img.width! * scale;

      console.log('final dimensions', finalHeight, finalWidth);
      ...
      if (localStorage.getItem(fileName + "ImageProperties")) {
        const imagePropertyValues: ImagePropertyValues = JSON.parse(
          localStorage.getItem(fileName + "ImageProperties")!
        );
        ...
        for (let i = 0; i < annaotationArray.length; i++) {
          if (annaotationArray[i].type === "circle") {
            ...
            line.globalCompositeOperation =
              annaotationArray[i].globalCompositeOperation;
